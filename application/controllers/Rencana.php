...
class rencana extends CI_Controller {
	...
	function generateNomorKwitansi(){
		$cTahun = date('Y');

		$sql = "SELECT iLastNUmber FROM cost.tabcount WHERE cCode='KUI' AND cTahun='".$cTahun."'";
		$query = $this->db->query($sql);
		if ($query->num_rows() > 0){
			$r = $query->row();
			$iLastNUmber = $r->iLastNUmber + 1;

			$sqlu = "UPDATE cost.tabcount SET iLastNUmber='".$iLastNUmber."' WHERE cCode='KUI' AND cTahun='".$cTahun."'";
		}else{
			$iLastNUmber = 1;
			$sqlu = "INSERT INTO cost.tabcount (cCode,cTahun,iLastNumber,vDesc) VALUES ('KUI','".$cTahun."',1,'Counter For Kuitansi')";
		}

		if ($iLastNUmber < 10){
			$iLastNUmber = "0".$iLastNUmber;
		}

		$autogenerated_code = "KUI-".$iLastNUmber."/".$cTahun;
	...
	function simpanDataKwitansi(){
		...
		if ($vNomorKwitansi==''){
			$vNomorKwitansi = $this->generateNomorKwitansi();
		}
		...
		foreach($post as $k=>$v) {
			...
			if (preg_match('/^vTransDari(.*)$/', $k, $match)) {
				$vTransDari[] = $v;
			}

			if (preg_match('/^vTransTujuan(.*)$/', $k, $match)) {
				$vTransTujuan[] = $v;
			}

			if (preg_match('/^trans_nBiaya(.*)$/', $k, $match)) {
				$trans_nBiaya[] = str_replace(",", "", $v);
			}

			if (preg_match('/^trans_vKeterangan(.*)$/', $k, $match)) {
				$trans_vKeterangan[] = $v;
			}
		...
		//transport
		foreach ($vTransDari as $key => $value) {
			foreach($value as $k=>$v) {
				$nNilaiKwitansi = $nNilaiKwitansi + $trans_nBiaya[0][$k];
				$data_kwitansi = array(
								'iCsDetailId' => $id,
								'iJenis'	  =>4,
								'vTransDari'  => $v,
								'vTransTujuan' => $vTransTujuan[0][$k],
								'vKeterangan' => $trans_vKeterangan[0][$k],
								'nJumlah'	  => $trans_nBiaya[0][$k]
							);
		...
		foreach($dpr_vPerincian as $key=>$value) {
			foreach($value as $k=>$v) {
				$nNilaiKwitansi = $nNilaiKwitansi + $dpr_nJumlah[0][$k];
				$data_dpr = array(
						'iCsDetailId' => $id,
						'vPerincian'  => $v,
						'nJumlah'	  => $dpr_nJumlah[0][$k]
					);
	...
	function cetakkwitansi(){
		...
		$tgl_dibuat 	=  $this->lib_util->tgl_indo(date('Y-m-d',strtotime($_GET['tgl_dibuatKwitansi'])));
	...
	function cetakperjadin(){
        ...
        $tgl_dibuat     = $this->lib_util->tgl_indo(date('Y-m-d',strtotime($_GET['tgl_dibuatperjadin'])));
        $terbilang  = $_GET['terbilang'];
	...
	function cetakdpr(){
        ...
        $tgl_dibuat     =  $this->lib_util->tgl_indo(date('Y-m-d',strtotime($_GET['tgl_dibuatdpr'])));
