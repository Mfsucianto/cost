...
class Grid{
	...
	private function getSearchCaption() {
		if($this->search_caption == NULL) {
			$this->search_caption = 'Cari '.$this->title;
		}
	...
	public function getControllerExtends() {
		...
		if(file_exists(APPPATH.'../../modules_erp/'.$urlpages[0].'/controllers/'.$urlpages[1].EXT)) {
			require_once APPPATH.'../../modules_erp/'.$urlpages[0].'/controllers/'.$urlpages[1].EXT;
		}
		else 
		{
			require_once APPPATH.'../modules/'.$urlpages[0].'/controllers/'.$urlpages[1].EXT;
	...
	public function getControllerExtendsUrl() {
		...
		if(file_exists(APPPATH.'../../modules_erp/'.$urlpages[0].'/controllers/'.$urlpages[1].EXT)) {
			$return = APPPATH.'../../modules_erp/'.$urlpages[0].'/controllers/'.$urlpages[1].EXT;
		}
		else {
			//$return = APPPATH.'../modules/'.$url.'/controllers/'.$url.EXT;
			$return = APPPATH.'../modules/'.$urlpages[0].'/controllers/'.$urlpages[1].EXT;
					...
					elseif($this->array_key_exists_r($c['name'], $this->change_ft)) {
						...
						$cs[$c['name']]['id'] = 'search_grid_'.$url.'_'.$cname;
							...
							if(is_array($rels) && count($rels) > 0) {
								...
								} else*/ if ($c['type'] == 'date' || $c['type'] == 'datetime' || $c['type'] == 'timestamp') {
									$cs[$c['name']] = array(
														'label'=>$c['label'],
														//'name'=>$c['name'],
                                                        'name'=>'search_grid_'.$url.'_'.$c['name'],
			...
			switch ($dataType) {
				...
				default :
					if (substr($dataType, 0, 7) == 'varchar') {
						...
						$this->ci->db->set($key, (int)$val);
					} else if (substr($dataType, 0, 5) == 'float') {
						$this->ci->db->set($key, (int)$val);
					} else {
						$this->ci->db->set($key, $val);
					}
	...
	public function json($paramArr) {
		...
		if(!empty($searchParam)) {
			foreach($searchParam as $sp => $val) {
				...
				if($val['type'] == 'datetime' || $val['type'] == 'timestamp') {
					if($val['value_start'] != '' && $val['value_end'] != '') {
						$value_start = $this->to_mysql($val['value_start']);
						$value_end = $this->to_mysql($val['value_end']);
	...
	public function getData(){
		...
		// coba insert 500 baris

		
		$resp = $this->pre_render_grid();

		$pk = $resp['pk'];
		
		$page = isset($paramArr['page']) ? $paramArr['page'] : 1;
		$rows = isset($paramArr['rows']) ? $paramArr['rows'] : 10;
		$start = isset($paramArr['start']) ? $paramArr['start'] : 1;
		$limit = isset($paramArr['limit']) ? $paramArr['limit'] : 10;
		$sortField = isset($paramArr['sidx']) ? $paramArr['sidx'] : $pk;
		$sortOrder = isset($paramArr['sord']) ? $paramArr['sord'] : 'ASC';
		$searchParam = isset($paramArr['searchFields']) ? $paramArr['searchFields'] : NULL;
		
		$table_name = $this->getTable();
		//count data
		$count = $this->ci->db->count_all_results($table_name);
		...
		if ($query->num_rows() > 0) {
			foreach($query->result_array() as $row) {
				$responce->rows[$i]['id']=$row['iInstansiId'];
    			$responce->rows[$i]['cell']=array($row['iInstansiId'],$row['vInstansiName']);
    			$i++;
